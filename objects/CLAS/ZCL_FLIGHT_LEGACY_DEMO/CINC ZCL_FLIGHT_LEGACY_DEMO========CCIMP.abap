...
CLASS lcl_booking_supplement_buffer DEFINITION FINAL CREATE PRIVATE FRIENDS ltc_booking_supplement.
  ...
  PRIVATE SECTION.
    ...
    METHODS _check IMPORTING is_booking_supplement  TYPE zbook_suppl
                             ...
                             iv_change_mode         TYPE zcl_flight_legacy_demo=>ty_change_mode
                   CHANGING  ct_messages            TYPE zif_flight_legacy=>tt_if_t100_message
    ...
    METHODS _check_currency_code IMPORTING is_booking_supplement  TYPE zbook_suppl
                                 ...
                                 CHANGING  ct_messages            TYPE zif_flight_legacy=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _determine IMPORTING iv_change_mode        TYPE zcl_flight_legacy_demo=>ty_change_mode
...
CLASS lcl_booking_supplement_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    LOOP AT it_booking_supplement INTO DATA(ls_booking_supplement_create) ##INTO_OK.
      ...
      IF _check( EXPORTING is_booking_supplement = ls_booking_supplement_create
                           iv_change_mode        = zcl_flight_legacy_demo=>change_mode-create
                 CHANGING  ct_messages           = et_messages ) = abap_false.
        RETURN.
      ENDIF.

      " standard determinations
      _determine( EXPORTING iv_change_mode        = zcl_flight_legacy_demo=>change_mode-create
  ...
  METHOD _update.
    ...
    LOOP AT it_booking_supplement ASSIGNING <s_booking_supplement_update>.
      ...
      IF _check( EXPORTING is_booking_supplement  = <s_buffer_booking_supplement>
                           ...
                           iv_change_mode         = zcl_flight_legacy_demo=>change_mode-update
                 CHANGING  ct_messages            = et_messages ) = abap_false.
        RETURN.
      ENDIF.
      ...
      _determine( EXPORTING iv_change_mode        = zcl_flight_legacy_demo=>change_mode-update
  ...
  METHOD _check_supplement.
    rv_is_valid = abap_true.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_booking_supplementx-supplement_id = abap_true ).
  ...
  METHOD _check_currency_code.
    rv_is_valid = abap_true.
    IF   ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-create AND is_booking_supplement-currency_code IS NOT INITIAL ) " Will be derived if initial
      OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_booking_supplementx-currency_code = abap_true ).
  ...
  METHOD _determine.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create.
...
CLASS lcl_booking_buffer IMPLEMENTATION.
  ...
  METHOD _check_booking_date.
    rv_is_valid = abap_true.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_bookingx-booking_date = abap_true ).

      " A. Booking Date must not be initial
      " B. When the record is created it must not be in the past
      IF is_booking-booking_date IS INITIAL OR is_booking-booking_date = '' OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-create AND is_booking-booking_date < cl_abap_context_info=>get_system_date( ) ).
        rv_is_valid = abap_false.
  ...
  METHOD _check_customer.
    rv_is_valid = abap_true.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_bookingx-customer_id = abap_true ).
  ...
  METHOD _check_flight.
    rv_is_valid = abap_true.
    IF     iv_change_mode = zcl_flight_legacy_demo=>change_mode-create
      OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update
  ...
  METHOD _check_currency_code.
    rv_is_valid = abap_true.
    IF   ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-create AND is_booking-currency_code IS NOT INITIAL ) " Will be derived if initial
      OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_bookingx-currency_code = abap_true ).
...
CLASS lcl_travel_buffer IMPLEMENTATION.
  ...
  METHOD _check_agency.
    rv_is_valid = abap_true.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-agency_id = abap_true ).
  ...
  METHOD _check_customer.
    rv_is_valid = abap_true.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-customer_id = abap_true ).
  ...
  METHOD _check_dates.
    ...
    " begin date
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-begin_date = abap_true ).
    ...
    " end date
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-end_date = abap_true ).
    ...
    " begin date < = end date
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-create OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-begin_date = abap_true )
                                                                  OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-end_date   = abap_true ).
  ...
  METHOD _check_status.
    rv_is_valid = abap_true.
    IF iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-status = abap_true.
  ...
  METHOD _check_currency_code.
    ...
    ENDIF.
    IF   iv_change_mode = zcl_flight_legacy_demo=>change_mode-create
      OR ( iv_change_mode = zcl_flight_legacy_demo=>change_mode-update AND is_travelx-currency_code = abap_true ).
